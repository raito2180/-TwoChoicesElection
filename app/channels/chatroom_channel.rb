class ChatroomChannel < ApplicationCable::Channel
  ...
  def render_message(chat)
    if chat.profile.user
      ApplicationController.render_with_signed_in_user(chat.profile.user, partial: 'chatrooms/mychat', locals: {chat: chat})
    else
      ApplicationController.render_with_signed_in_user(chat.profile.user, partial: 'chatrooms/youchat', locals: {chat: chat})
    end
  end  
